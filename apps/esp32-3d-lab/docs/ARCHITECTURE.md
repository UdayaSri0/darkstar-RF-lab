...
Project state contains wires, measurements, and notes. Mutating operations copy the previous serialisable state into a bounded 50-entry undo history, rebuild project overlays, and autosave after a short debounce. Camera and UI settings use independent keys.

No state is attached to `window`, and no service worker or cross-application messaging is registered.

## Error handling

Startup is guarded in `main.ts`. A missing DOM prerequisite or WebGL startup error replaces the root with a recoverable error panel. Invalid stored data and invalid imported project files fail safely without changing the current project.
